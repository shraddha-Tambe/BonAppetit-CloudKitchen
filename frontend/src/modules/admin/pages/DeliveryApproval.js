...
import { Search, Check, X, Trash2, MapPin, Phone, Mail, Calendar, Bike } from "lucide-react";
                ...
                {filteredDeliveryBoys.map((deliveryBoy) => {
                    ...
                    <div key={deliveryBoy.id} className="stat-card">
                        <div className="flex items-start justify-between mb-4">
                            <div>
                                <h3 className="font-semibold text-lg text-foreground">{deliveryBoy.name || "Unknown"}</h3>
                                <p className="text-sm text-muted-foreground">{deliveryBoy.vehicle || "N/A"}</p>
                            </div>
                            <span
                        ...
                        {status === "pending" && (
                            <div className="flex gap-2 mt-4 pt-4 border-t border-border">
                                <Button
                                    onClick={() => handleApprove(deliveryBoy.id, deliveryBoy.name)}
                                ...
                                <Button
                                    onClick={() => handleReject(deliveryBoy.id, deliveryBoy.name)}
                                    variant="destructive"
                                    className="flex-1"
                                >
                        ...
                        {status !== "pending" && (
                            <div className="flex gap-2 mt-4 pt-4 border-t border-border">
                                <Button
                                    onClick={() => handleRemove(deliveryBoy.id, deliveryBoy.name)}
                                    variant="outline"
                                    className="flex-1"
                                >
